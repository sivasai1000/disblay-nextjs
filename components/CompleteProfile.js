...
export default function CompleteProfile({ businessId, onFinish }) {
...
useEffect(() => {
  if (data?.status === "success") {
    ...
    Object.entries(data.response || {}).forEach(([key, val]) => {
      clean[key] =
  ...
  if (page === 2) {
    if (
      ...
      await Swal.fire({
        ...
        text: "Please fill the business address field.",
      });
      return;
    }
  }
  ...
  if (save && isDirty) {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Update Failed",
        text: "Could not save business details. Please try again.",
      });
      return;
    }
  }
 ...
 const handleGetLocation = async () => {
  ...
  if (result.latitude && result.longitude) {
    setFormData((prev) => ({
      ...
      direction: result.address, // full address
    }));
  } else {
    await Swal.fire({
      icon: "error",

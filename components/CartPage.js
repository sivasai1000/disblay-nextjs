...
// ❗ FIX: Do not run API or UI until businessId is ready
if (businessId === null) {
  return (
...
// NOW YOUR API HOOK WILL RECEIVE A VALID businessId
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      const payload = {
        package_id: isCombo
          ? firstItem?.id
          : currentBusiness?.id || currentBusiness?.business_id,
        ...
        notification_type: isCombo
          ...
          ? "Products"
          : "Services",
        delivery_type: currentBusiness?.delivery_type || "",

...
useEffect(() => {
  const saved = sessionStorage.getItem("comboDetailState");
...
const handleCreateProduct = async (e) => {
  ...
  for (let i = 0; i < productImages.length; i++) {
    ...
    if (file) {
      ...
      const key = i === 0 ? "product_logo" : `logo${i}`;
      const url = preview.replace(process.env.NEXT_PUBLIC_API_URL + "/", "");
...
const handleUpdatePackage = async (e) => {
  ...
  try {
    ...
    // If your API returns a status, optionally gate on it:
    // if (result?.status !== "success") throw new Error(result?.msg || "Update failed");
...
const handleUpdate = async (e) => {
  ...
  for (let i = 0; i < productImages.length; i++) {
    ...
    } else if (productImagesPreview[i]) {
      const path = productImagesPreview[i].replace(
...
const handleUpdateService = async (e) => {
  ...
  for (let i = 0; i < serviceImages.length; i++) {
    const key = i === 0 ? "service_image" : `image${i}`;
    let file = serviceImages[i];
    ...
    if (file) {
      ...
      formData.append(key, file);
    } else if (serviceImagesPreview[i]) {
      const path = serviceImagesPreview[i].replace(
        process.env.NEXT_PUBLIC_API_URL + "/",
        ""
      );
      formData.append(key, path);
...
const handledeleteproduct = async (id) => {
  ...
  if (!confirm.isConfirmed) return; // ❌ User cancelled delete
...
<div className="d-flex justify-content-between align-items-center">
  ...
  <img
    ...
    alt={packageInfo.package_name || "name"}
    style={{
      width: "80px",
      height: "80px",
      borderRadius: "12px",
...
{packageInfo?.isPaid !== "1" && categories.length > 0 && (
  ...
  onClick={async () => {
    ...
    // ✅ Case 1: Free subscription allowed
...
const durationDays =
  packageInfo?.start_date && packageInfo?.end_date
    ? Math.ceil(
        (new Date(packageInfo.end_date) -
          new Date(packageInfo.start_date)) /
          (1000 * 60 * 60 * 24)
      )
    : 365;

// Save state data because Next.js cannot pass router.state
sessionStorage.setItem(
  "successState",
  JSON.stringify({ shareLink, durationDays })
);
...
router.push("/success");
    ...
    // ✅ Case 2: Normal flow → PlanSelector
  const planState = {
  packageCode: packageInfo?.package_code,
  packageId,
  businessId: combo?.business_id,
...
sessionStorage.setItem("planSelectorState", JSON.stringify(planState));

// ALSO save individual keys (QRPage needs these)
sessionStorage.setItem("packageCode", String(packageInfo?.package_code || ""));
sessionStorage.setItem("packageId", String(packageId || ""));
sessionStorage.setItem("businessId", String(combo?.business_id || ""));
sessionStorage.setItem("plan_for", "package");
sessionStorage.setItem("type", "Subscription");
sessionStorage.setItem("gstNo", "");
sessionStorage.setItem("durationDays", "");
sessionStorage.setItem("planId", "");
sessionStorage.setItem("amount", "");
sessionStorage.setItem("shareLink", "");
...
>
  <span
    style={{ fontWeight: "600", color: "#FFFFFF", fontSize: "20px" }}
  >
    Submit
  </span>
</Button>
...
<Form.Group className="mb-3 position-relative">
  ...
  <Form.Control
    ...
    required
    maxLength={25} // ✅ restricts to 25 characters
    value={productName}
    onChange={(e) => setProductName(e.target.value)} // ✅ live counter
  ...
  <Form.Control
    ...
    onChange={(e) => setProductDescription(e.target.value)} // ✅ live counter
  ...
  <span
    style={{
      ...
      right: "12px",
      fontSize: "12px",
      color: productDescription.length >= 1000 ? "red" : "#6c757d",
      pointerEvents: "none",
    }}
  >
...
>
  {posterPreview ? (
    ...
    <>
      {/* Default empty state */}
      <img src={imageplus1} width={32} height={32} style={{ opacity: 1 }} alt="plus" />
...
>
  {pdfFile ? (
    <>
      ...
      <img src={viewpdf} width={40} height={40} style={{ opacity: 1 }} alt="pdf" />
      
    </>
  ) : (
    <>
      {/* Default empty state */}
      <img src={imagepdf} width={32} height={32} style={{ opacity: 1 }} alt="pdf" />
      ...
      <Form.Group className="mb-3 position-relative">
        ...
        <span
          style={{
            ...
            right: "12px",
            fontSize: "12px",
            color: catName.length >= 30 ? "red" : "#6c757d",
            pointerEvents: "none",
          }}
        >
          {catName.length}/30
      ...
      <Form.Group className="mb-3 position-relative">
        ...
        <span
          style={{
            ...
            right: "12px",
            fontSize: "12px",
            color: catSubtitle.length >= 45 ? "red" : "#6c757d",
            pointerEvents: "none",
          }}
        >
          {catSubtitle.length}/45
        </span>
      </Form.Group>
      ...
      <Form.Group className="position-relative">
        ...
        <span
          style={{
            ...
            right: "12px",
            fontSize: "12px",
            color: catNote.length >= 60 ? "red" : "#6c757d",
            pointerEvents: "none",
          }}
        >
          {catNote.length}/60
...
>
  ...
  const stateData = {
    ...
    assignedIds: allProducts.map((p) => p.id),
    assignedIdservice: allServices.map((s) => s.id),
  };

  sessionStorage.setItem("masterState", JSON.stringify(stateData));

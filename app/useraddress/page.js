...
export default function UserSavedAddress({cart, setCart, business, setBusiness }) {
let user = null;
let userId = null;
 ...
 const handleLocation = () => {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      // ✅ If user denies permission or any error occurs
      async (err) => {
        await Swal.fire({
          icon: "error",
 ...
 const handleManualLatLong = async () => {
  ...
  if (!lat || !lng) {
    await Swal.fire({
      ...
      text: "Use correct format: lat,long (e.g. 17.49224,78.39919)",
    });
    return;
  }
  ...
  await Swal.fire({
    ...
    text: "Direction fetched successfully!",
    timer: 1200,
